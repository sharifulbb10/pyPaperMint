...
IN THE TERMINAL, RUN THIS SCRIPT:
    python3 fetch.py --search 'textile' --start 0 --maximum 50 --db arxiv --file_name yourFileName.xlsx
...
# args = parser.parse_args(['--search', 'ai textile', '--start', '0', '--maximum', '4', '--db', 'others'])
...
def arxiv_search():
    ...
    for index, entry in enumerate(feed.entries):
        ...
        authors.append((to_text(names).strip()).replace('\n', ' '))
...
def openAlex_search():
    url = f"https://api.openalex.org/works?search=({prepare_search_others(args.search)})&page={int(args.start)+1}&per-page={int(args.maximum)-int(args.start)}"
    print(url,'\n')
    ...
    for index, entry in enumerate(data['results']):
        ...
        for authors in entry['authorships']:
            authors_of_paper.append(f"{authors['author']['display_name']} ({authors['raw_affiliation_strings'][0]})")
